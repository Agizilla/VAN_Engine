// =============================================================================
// BUTLER MEDIA DECK — Voice Synthesis Playback Toolbar
// Injects a media control deck into Gemini / DeepSeek / AI chat platforms
// for reading assistant messages aloud with configurable voice, rate, pitch.
//
// Usage: Paste into browser DevTools Console (F12) on the target page.
// Or save as a bookmarklet: javascript:(function(){...})()
//
// Features:
// - Multi-platform DOM detection (Gemini, DeepSeek, generic .message-content)
// - Unified voice config sync (chrome.storage.local persistence)
// - Play / Pause / Stop / Prev / Next / First / Last track navigation
// - Copy current message text to clipboard (strips code blocks)
// - Settings modal: female/male voice selection, rate, pitch, preview
// - MutationObserver: auto-reinjects toolbar as new messages load
// - Layout brake: unrolls Gemini's maxWidth stacking
// - Trusted Types CSP compliance
// - Safe word filtered: skips "can make mistakes" footers
//
// Author: OpenCode / VAN_Engine Collective
// Location: docs/daily-scripts/butler_media_deck.js
// =============================================================================
...
(function() {
    ...
    function launchSettingsModal() {
        ...
        overlay.innerHTML = escapePolicy.createHTML(`
            <div class="butler-modal">
                ...
                <div class="butler-field" id="field-female-dropdown"><label>Female Voice Profiles</label><select id="butler-female-select"><option value="">-- Choose Profile --</option>${femaleOptions}</select></div>
                <div class="butler-field" id="field-male-dropdown"><label>Male Profiles</label><select id="butler-male-select"><option value="">-- Choose Profile --</option>${maleOptions}</select></div>
                <div class="butler-field">
                    <label>Speech Rate</label>
                    <div class="butler-slider-container"><input type="range" id="butler-rate" min="0.5" max="2" step="0.05" value="${voiceConfig.rate}"><span id="butler-rate-txt">${voiceConfig.rate}x</span></div>
                </div>
                <div class="butler-field">
                    <label>Voice Pitch</label>
                    <div class="butler-slider-container"><input type="range" id="butler-pitch" min="0.5" max="1.5" step="0.05" value="${voiceConfig.pitch}"><span id="butler-pitch-txt">${voiceConfig.pitch}</span></div>
                </div>
                <div class="butler-actions">
                    <button class="butler-btn butler-btn-test" id="butler-test">\uD83C\uDF99\uFE0F Preview</button>
                    <button class="butler-btn butler-btn-cancel" id="butler-close">Cancel</button>
                    <button class="butler-btn butler-btn-apply" id="butler-save">Apply</button>
                </div>
            </div>
        `);
        ...
        rSlider.oninput = () => document.getElementById('butler-rate-txt').innerText = rSlider.value + 'x';
        pSlider.oninput = () => document.getElementById('butler-pitch-txt').innerText = pSlider.value;

        document.getElementById('butler-test').onclick = () => {
            window.speechSynthesis.cancel();
        ...
        document.getElementById('butler-close').onclick = () => { window.speechSynthesis.cancel(); overlay.remove(); };
    }
    ...
    function injectMediaDeck() {
        ...
        deck.querySelectorAll('.butler-btn-action').forEach(btn => {
            btn.onclick = (e) => {
                e.preventDefault();
        ...
        deck.querySelector('#butler-deck-play-btn').onclick = (e) => { e.preventDefault(); togglePlayPause(); };
        deck.querySelector('#butler-deck-copy-btn').onclick = (e) => { e.preventDefault(); copyCurrentTrackText(); };
        deck.querySelector('#butler-deck-gear-btn').onclick = (e) => { e.preventDefault(); launchSettingsModal(); };
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        for (let m of mutations) {
            if (m.target && m.target.id !== 'butler-deck-display-screen' && !m.target.closest?.('.butler-media-deck')) {
