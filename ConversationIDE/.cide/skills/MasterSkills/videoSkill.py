...
__meta__ = {
    ...
    "dateLastModified": "2026-06-07 12:12",
    ...
    "update_list": [
        "2026-06-07 v1.0.0 — Initial merge: Video Frame (base, 69 KB), TalkingAvatar (54 KB), MovieDirector (17 KB), Video Blink Counter, YoutubeDL GUI.",
        "    Extracted 15 capability categories across 60 public methods.",
        "    Capabilities: FaceDetection, EyeDetection, BlinkDetection, FrameExtraction, TalkingAvatar, MouthTracking, VideoTrimming, VideoConcat, VideoDownload, GIFCreation, MusicVideoGeneration, WebcamCapture, VideoEffects, SpeedChange, AudioExtraction."
    ]
}
...
class FrameExtractor:
    ...
    def extract_every_n(self, video_path: Path, output_dir: Path,
                        n: int = 30, format: str = "jpg") -> list[Path]:
        output_dir.mkdir(parents=True, exist_ok=True)
        frames = []
        with VideoReader(video_path) as reader:
            idx = 0
...
class VideoToGIF:
    ...
    def convert(self, video_path: Path, output_path: Path | None = None,
                start_time: float = 0, duration: float | None = None) -> Path:
        if output_path is None:
...
class VideoDownloader:
    ...
    def get_info(self, url: str) -> dict:
        try:
            ...
            raise SkillError("yt-dlp required")
...
class VideoSkill:
    ...
    def extract_frames(self, video_path: Path, output_dir: Path | None = None,
                       every_n: int = 1) -> list[Path]:
        extractor = FrameExtractor()
        out = output_dir or CONFIG["output_dir"] / "frames" / video_path.stem
        if every_n > 1:
            return extractor.extract_every_n(video_path, out, every_n)
        return extractor.extract_all(video_path, out)
    ...
    def make_gif(self, video_path: Path, output_path: Path | None = None,
        ...
        return gif.convert(video_path, output_path, start, duration)
