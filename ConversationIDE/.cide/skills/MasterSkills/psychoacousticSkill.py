...
__meta__ = {
    ...
    "update_list": [
        ...
        "    FlavorEngine: circadian-rhythm ambient soundscapes (ocean, rain, fireplace, forest, night).",
        "    DreamProcessor: REM sleep detection via breathing/movement audio analysis + binaural beat lucid triggers.",
        "    TherapyEngine: audio-based grief/emotional distress detection with Solfeggio-frequency empathetic responses.",
        "    TearTrigger: 4-phase emotional release sequence (opening, deepening, release, integration) with safety cooldowns.",
        "    Capabilities: ambient_soundscapes, rem_detection, lucid_triggers, grief_detection, emotional_release."
    ]
}
...
class FlavorEngine:
    ...
    def generate_ambient(self, duration_s: float = 0.1) -> np.ndarray:
        ...
        sr = self.config.get("sample_rate", CONFIG["sample_rate"])
...
class DreamProcessor:
    def __init__(self, config: dict | None = None):
        ...
        self.rem_detection_enabled = cfg.get("rem_detection_enabled", True)
        self.min_rem_duration = cfg.get("min_rem_duration_minutes", 5) * 60
        self.binaural_frequency = cfg.get("binaural_frequency", 6.0)
        self.trigger_volume = cfg.get("trigger_volume", 0.15)
        self.reality_check_interval = cfg.get("reality_check_interval_minutes", 20) * 60
        self.emergency_wake = cfg.get("emergency_wake_enabled", True)
...
class TherapyEngine:
    def __init__(self, config: dict | None = None):
        cfg = config or CONFIG
        self.grief_detection_enabled = cfg.get("grief_detection_enabled", True)
        self.empathy_volume = cfg.get("empathy_response_volume", 0.2)
        self.silence_period = cfg.get("silence_period_seconds", 30)
        self.morning_report_enabled = cfg.get("morning_report_enabled", True)
        self.emotional_log_retention = cfg.get("emotional_log_retention_days", 7)
        ...
        self.sample_rate = cfg.get("sample_rate", CONFIG["sample_rate"])
...
class TearTrigger:
    def __init__(self, config: dict | None = None):
        cfg = config or CONFIG
        self.enabled = False
        self.max_intensity = cfg.get("max_intensity", 0.3)
        self.cooldown_hours = cfg.get("cooldown_hours", 24)
        self.max_duration_minutes = cfg.get("max_duration_minutes", 15)
        self.session_active = False
        self.session_start_time: float | None = None
        self.last_session_time: float | None = None
        self.suppression_score = 0.0
        self.suppression_threshold = cfg.get("suppression_threshold", 0.7)
...
class PsychoacousticSkill:
    ...
    def flavor(self) -> FlavorEngine:
        if self._flavor is None:
            self._flavor = FlavorEngine(CONFIG)
        return self._flavor

    def get_ambient(self, flavor: str = "neutral", duration_s: float = 0.1) -> np.ndarray:
        self.flavor.set_flavor(flavor)
        return self.flavor.generate_ambient(duration_s)
    ...
    def dream(self) -> DreamProcessor:
        if self._dream is None:
            self._dream = DreamProcessor(CONFIG)
        return self._dream
    ...
    def generate_binaural_beat(self, duration_s: float = 5.0) -> np.ndarray:
        return self.dream.generate_binaural_beat(duration_s)
    ...
    def therapy(self) -> TherapyEngine:
        if self._therapy is None:
            self._therapy = TherapyEngine(CONFIG)
    ...
    def generate_opening(self, duration_s: float = 60.0) -> np.ndarray:
        return self.tear.generate_opening_phase(duration_s) if self.tear.session_active else np.array([])

    def generate_deepening(self, duration_s: float = 180.0) -> np.ndarray:
        return self.tear.generate_deepening_phase(duration_s) if self.tear.session_active else np.array([])

    def generate_release(self, duration_s: float = 120.0) -> np.ndarray:
        return self.tear.generate_release_phase(duration_s) if self.tear.session_active else np.array([])

    def generate_integration(self, duration_s: float = 120.0) -> np.ndarray:
        return self.tear.generate_integration_phase(duration_s) if self.tear.session_active else np.array([])
