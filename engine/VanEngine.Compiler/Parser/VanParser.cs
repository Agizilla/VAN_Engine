...
using VanEngine.Compiler.Lexer;

namespace VanEngine.Compiler.Parser;
                    ...
                    label.Equals("STATE", StringComparison.Ordinal))
                {
                    ...
                    if (env != null)
                    {
                        if (label.Equals("STATE", StringComparison.Ordinal))
                            env.BlockType = VanEngine.Core.VAN.VanBlockType.State;
                        envelopes.Add(env);
                    }
                }
                else
    ...
    private AstEnvelope? ParseTransition()
    {
        ...
        var envelope = new AstEnvelope
        {
            ...
            BlockType = label.Equals("STATE", StringComparison.OrdinalIgnoreCase)
                ? VanEngine.Core.VAN.VanBlockType.State
                : VanEngine.Core.VAN.VanBlockType.Transition
        };
