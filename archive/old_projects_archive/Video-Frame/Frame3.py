...
try:
    ...
    print("Error: Required dlib or haarcascade files not found. Make sure 'shape_predictor_68_face_landmarks.dat' and 'haarcascade_frontalface_default.xml' are in the script directory.")
    exit()

if not face_cascade.empty() and predictor:
    # Indices for the eyes in the Dlib 68-point model
    (R_START, R_END) = (36, 42)
    (L_START, L_END) = (42, 48)
else:
...
class TransparentDetector(tk.Tk):
    ...
    def __init__(self):
        ...
        # Canvas background color should match main_frame or be set to a known color 
        # to ensure proper alpha blending.
    ...
    def eye_aspect_ratio(self, eye):
        """Computes the Eye Aspect Ratio (EAR) for a given eye landmark set."""
        # Vertical distances
    ...
    def update_timer_and_bpm(self):
        ...
        # Use a base time to avoid division by zero or overly high initial BPM
    ...
    def capture_screen_area(self):
        """Captures the portion of the screen covered by the window."""
    ...
    def draw_detections(self, frame_bgr, faces, avg_ear):
        """
        Draws only the detection overlays (boxes/landmarks) on the transparent canvas.
        The screen image itself is NOT drawn to prevent ghosting.
        """
        ...
        # Calculate scaling factors to map OpenCV coordinates to Tkinter canvas coordinates
        ...
        for (x, y, w, h) in faces:
            ...
            # Re-run Dlib landmark calculation for precision and scaling
    ...
    def process_blinks(self, avg_ear):
        """Updates blink count based on Eye Aspect Ratio (EAR)."""
    ...
    def run_detection(self):
        """The main loop for screen capture and detection."""
        ...
        # 1. Face Detection
        gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
        # Haar cascade is used for fast initial face detection
        faces = face_cascade.detectMultiScale(gray, 1.1, 4)

        avg_ear = 1.0
        if len(faces) > 0:
            # Process only the largest/first detected face
            (x, y, w, h) = faces[0] 
            rect = dlib.rectangle(x, y, x + w, y + h)
            
            # Dlib is used for precise landmark detection
            shape = predictor(gray, rect)
            points = np.array([[p.x, p.y] for p in shape.parts()])
